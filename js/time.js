var time = {
	...
	populate_table : function() {
		$.ajax({
			url: time.config.url + 'workevent/payperiod/' + time.current_period + '/',
			method: 'GET',
			dataType: 'JSON',
			beforeSend: function (request) {
				request.setRequestHeader("AUTHENTICATE", time.config.token);
			},
	...
	addEntry : function() {
		...
		$('#addEntry').modal('hide');
		$.ajax({
			url: time.config.url  + 'workevent/add/',
			method: 'POST',
			data: data,
			beforeSend: function (request) {
				request.setRequestHeader("AUTHENTICATE", time.config.token);
	...
	update : function() {
		...
		$.ajax({
			url: time.config.url + 'workevent/update/' + id + '/',
			method: 'POST',
			data: data,
			beforeSend: function (request) {
				request.setRequestHeader("AUTHENTICATE", time.config.token);
